...
function slugRoomId(raw: string): string {
  return raw.replace(/[^a-zA-Z0-9-_]/g, '').slice(0, 48) || 'StudYearTutorRoom';
}

export function TutorLiveClassroom() {
  const { user } = useAuth();
  ...
  const roomName = useMemo(() => {
    const base = user?.uid ? `StudYear-${slugRoomId(user.uid)}` : 'StudYear-Tutor';
  ...
  const meetUrl = useMemo(() => {
    ...
    const displayName = userProfile?.name?.trim() || 'StudYear Tutor';
  ...
  const copyLink = async () => {
    try {
      ...
      setCopied(true);
      toast({ title: 'Link copied', description: 'Share this room URL with your student.' });
      setTimeout(() => setCopied(false), 2000);
    } catch {
      toast({ variant: 'destructive', title: 'Copy failed', description: 'Copy the URL from the address bar inside the room.' });
    }
  };
    ...
    <div className="space-y-4">
      <Card className="tutor-panel">
        <CardHeader>
          ...
          <CardDescription>
            HD video and screen share powered by Jitsi. Share the invite link with your student — they can join from any
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="room-suffix">Session label (optional)</Label>
              <Input
                id="room-suffix"
                value={roomSuffix}
                onChange={(e) => setRoomSuffix(e.target.value)}
                placeholder="e.g. maths-gcse"
                maxLength={24}
              />
      ...
      <div className="overflow-hidden rounded-xl border bg-black shadow-lg">
        <iframe
          title="StudYear tutor live classroom"
          src={meetUrl}
