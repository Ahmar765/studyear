...
export function subscriptionOptionsForRole(role: string): SubscriptionType[] {
  switch (role) {
    case 'PARENT':
      return ['FREE', 'PARENT_PRO', 'PARENT_PRO_PLUS', 'PARENT_ELITE'];
    case 'STUDENT':
      return ['FREE', 'STUDENT_PREMIUM', 'STUDENT_PREMIUM_PLUS'];
    case 'PRIVATE_TUTOR':
...
export function isActiveParentPlan(type: SubscriptionType): boolean {
  return type === 'PARENT_PRO' || type === 'PARENT_PRO_PLUS' || type === 'PARENT_ELITE';
}
